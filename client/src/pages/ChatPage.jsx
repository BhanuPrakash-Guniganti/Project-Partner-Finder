...
const ChatPage = () => {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      if (isReset) {
        // Explicit reset requested from Navbar Chat link
        clearChatSelection();
        ...
        // Default general Chat page landing view (no chat selected)
        clearChatSelection();
      }
    } catch (err) {
  ...
  // Helper to safely append a message without duplicate entries
  ...
  // Collect unique direct contacts across teams
  const contactList = [];
  teams.forEach(t => {
    t.members?.forEach(m => {
      if (m.userId && m.userId._id !== user._id) {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-gray-100 flex flex-col justify-between">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full space-y-6 flex-1">
        <div className="glass-panel p-4 sm:p-6 rounded-2xl border border-gray-800 flex flex-col md:flex-row h-[650px] gap-4 shadow-2xl">
          
          {/* Contacts & Project Teams Sidebar */}
          <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-r border-gray-800 pr-0 md:pr-4 flex flex-col justify-between h-full space-y-4">
                  ...
                  teams.map(teamObj => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[10px] bg-gray-800 text-gray-400 px-1.5 py-0.5 rounded font-mono">Team</span>
                      </div>
                    );
                  })
                )}
                  ...
                  contactList.map(contact => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-2.5 rounded-xl cursor-pointer transition-all flex items-center space-x-3 text-xs ${
                          isSelected 
                            ? 'bg-cyan-950/50 text-cyan-300 border border-cyan-700 font-semibold shadow-md' 
                            : 'hover:bg-gray-900/80 text-gray-300 border border-transparent'
                        }`}
            ...
            {(selectedUser || selectedProject) && (
              <button
                ...
                className="w-full py-2 bg-gray-900 hover:bg-gray-800 border border-gray-800 rounded-xl text-xs font-semibold text-gray-400 hover:text-white transition-colors"
              >
          ...
          <div className="flex-1 flex flex-col justify-between h-full min-w-0">
            {chatType === 'direct' && selectedUser ? (
              <>
                ...
                <div className="p-3 border-b border-gray-800 font-bold text-xs sm:text-sm text-white flex items-center justify-between">
                  <div className="flex items-center space-x-2.5 truncate">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 p-4 bg-gray-950/40 rounded-xl border border-gray-800/80 my-2">
                  {messagesLoading ? (
                    ...
                    <div className="flex items-center justify-center h-full text-xs text-gray-500">
                      No direct messages yet. Say hello to start the conversation!
                    </div>
                  ) : (
                    messages.map((msg, idx) => {
                      ...
                      return (
                        <div key={idx} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                          <div className={`p-3 rounded-xl max-w-sm text-xs shadow-md ${
                ...
                <form onSubmit={handleSend} className="flex gap-2 pt-1">
                  ...
                  />
                  <button type="submit" className="gradient-btn px-5 py-2.5 rounded-xl font-bold text-white text-xs shadow-lg flex items-center justify-center">
              ...
              <>
                ...
                <div className="p-3 border-b border-gray-800 font-bold text-xs sm:text-sm text-white flex items-center justify-between">
                  ...
                  <span className="text-[10px] px-2 py-0.5 rounded bg-indigo-950/60 text-indigo-300 border border-indigo-800 font-mono">
                    Team Chat
                  </span>
                </div>
                ...
                <form onSubmit={handleSend} className="flex gap-2 pt-1">
                  <input
                    type="text"
                    placeholder={`Broadcast team message to ${selectedProject.title || 'Project'}...`}
                    value={newMessage}
                    onChange={(e) => setNewMessage(e.target.value)}
                    className="flex-1 bg-gray-900 border border-gray-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-cyan-500"
                  />
              ...
              <div className="flex flex-col items-center justify-center h-full text-center p-6 space-y-4 bg-gray-950/20 rounded-xl border border-gray-800/60">
                ...
                <div className="space-y-1 max-w-md">
                  <h3 className="text-base font-bold text-white">General Chat & Messaging Hub</h3>
                  <p className="text-xs text-gray-400 leading-relaxed">
                    Select a project team or a teammate from the left sidebar to start a conversation.
                  </p>
                </div>
              </div>
            )}
